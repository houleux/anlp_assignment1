"""Evaluate trained C1-C5 models with memory monitoring and batch_size=1 for safety."""
...
def get_memory_usage() -> Dict[str, float]:
    ...
    return {
        "rss_gb": info.rss / 1e9,  # Resident set size (actual memory)
        "vms_gb": info.vms / 1e9,  # Virtual memory
    }
...
def evaluate_tokenized(model, loader, tokenizer, device, max_new_tokens: int, max_samples: int):
    """Evaluate tokenized models with batch_size=1 generation to avoid OOM."""
    ...
    print(f"  Starting evaluation (batch_size=1, max_new_tokens={max_new_tokens})", flush=True)
    mem_start = get_memory_usage()
    print(f"  Memory at start: {mem_start['rss_gb']:.2f} GB RSS", flush=True)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            for sample_idx in range(keep):
                try:
                    src = batch["encoder_input"][sample_idx:sample_idx+1].to(device)
                    src_valid = batch["encoder_mask"][sample_idx:sample_idx+1].bool().to(device)
                    target = batch["decoder_input"][sample_idx]
                    target_valid = batch["decoder_padding_mask"][sample_idx].bool()
                    ...
                    # Decode
                    reference_ids = target[target_valid].tolist()
                    hypothesis_ids = strip_token_ids(generated[0].tolist())
                    ...
                    hypothesis_tokens.append(hypothesis_text.split())
                    ...
                    if processed % 10 == 0 or processed == max_samples:
                        ...
                        print(
                            f"  Processed {processed}/{max_samples} samples | "
                            f"Memory: {mem_now['rss_gb']:.2f} GB RSS | "
                            f"Bit acc: {bit_scores[-1]:.3f}",
                            flush=True
                        )
                
                except RuntimeError as e:
                    if "out of memory" in str(e).lower():
                        print(f"  ⚠️ OOM at sample {processed}: {e}", flush=True)
                        torch.cuda.empty_cache() if torch.cuda.is_available() else None
                        # Continue with remaining samples skipped
                        break
                    else:
                        raise
    ...
    exact = sum(
        ref == hyp 
        for ref, hyp in zip(reference_tokens, hypothesis_tokens)
    )
...
def evaluate_bytes(model, loader, device, max_samples: int):
    """Evaluate byte-level models with batch_size=1 generation."""
    bit_scores: List[float] = []
    distances: List[int] = []
    exact = 0
    processed = 0

    print(f"  Starting evaluation (batch_size=1)", flush=True)
    mem_start = get_memory_usage()
    print(f"  Memory at start: {mem_start['rss_gb']:.2f} GB RSS", flush=True)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            for sample_idx in range(keep):
                try:
                    source = batch["encoder_input"][sample_idx:sample_idx+1].to(device)
                    source_mask = batch["encoder_mask"][sample_idx:sample_idx+1].bool().to(device)
                    target = batch["decoder_input"][sample_idx]
                    target_valid = batch["decoder_padding_mask"][sample_idx]
                    target_length = target_valid.sum().item()
                    ...
                    reference = strip_byte_ids(target[:int(target_length)].tolist())
                    hypothesis = strip_byte_ids(generated[0].tolist())
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate trained C1-C5 checkpoints (batch_size=1, memory-safe).")
    ...
    for model_name in model_names:
        ...
        try:
            ...
            print(f"  ⚠️ Skipping {model_name}: {e}", flush=True)
            continue
        ...
        if model_name in TOKENIZED_MODELS:
            ...
            # Always use batch_size=1 for safety
            loader = DataLoader(dataset, batch_size=1, shuffle=False, collate_fn=collate_fn_tokenized)
            ...
            loader = DataLoader(dataset, batch_size=1, shuffle=False, collate_fn=collate_fn_bytes)
